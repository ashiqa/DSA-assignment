...
void fitting(bdptr &B,int k,bdptr &curr,bdptr left,bdptr right,bdptr &parent){
    ...
    /*START  OF  NODE  SPLIT : OVERFLOW*/
        ...
        for(int i=0;i<=opptr->cnt+1;++i)opptr->cptr[i]=NULL;
        int l=-1;
        ...
        //splitting  of overload node into curr and newn
        parent->cptr[0]->cnt=opptr->cnt/(sibcnt+1);
        for(int i=0;i<parent->cptr[0]->cnt;++i){
            parent->cptr[0]->keys[i]=opptr->keys[i];
            parent->cptr[0]->cptr[i]=opptr->cptr[i];
        }
        parent->keys[0]=opptr->keys[opptr->cnt/(sibcnt+1)];
        parent->cptr[1]->cnt=opptr->cnt/(sibcnt+1);
        for(int i=0;i<parent->cptr[1]->cnt;++i){
            parent->cptr[1]->keys[i]=opptr->keys[1+i+parent->cptr[1]->cnt];
        }
        parent->cnt++;
        parent->keys[parent->cnt-1]=opptr->keys[2*3+1];
        bdptr newn;
        newn=new(bdnode);
        newn->cnt=opptr->cnt-8;
        for(int i=0;i<m;++i)newn->cptr[i]=NULL;
         for(int i=0;i<newn->cnt;++i){
             newn->keys[i]=opptr->keys[8+i];
         }
